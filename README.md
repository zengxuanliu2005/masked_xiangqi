...
`npm run dev` 初始只监听本机；打开本机页面后，可直接用设置中的「允许同一网络的设备连接」开关，在同一个进程内切换本机/LAN 监听。也可以加 `--lan` 让进程一启动就进入 LAN 模式；需要在另一台设备上使用开发模式 HMR 时应采用这种方式：
...
- 端口占用：停止占用 3001 的旧进程，或用 `PORT` 换端口；开发模式还会额外占用 Vite 的 HMR 端口 24678。
...
- 开发与生产模式都是单一来源（3001 端口同时提供网页与 API），因此局域网对战在两种模式下都可用，界面上的开关也一样生效。开发模式下 HMR 的 WebSocket 端口（24678）跟随进程启动时的模式：以 loopback 启动时只绑定 `127.0.0.1`，以 `--lan` 启动时才对局域网开放。
