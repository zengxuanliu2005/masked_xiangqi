...
开发模式是单进程：Express 监听 3001，并以中间件方式挂载 Vite（保留 HMR），网页与 API 同源。
...
`npm run dev` 初始只监听本机；打开本机页面后，可直接用设置中的「允许同一网络的设备连接」开关，在同一个进程内切换本机/LAN 监听。也可以加 `--lan` 让应用 listener 一启动就进入 LAN 模式：
...
- 端口占用：停止占用 3001 的旧进程，或用 `PORT` 换端口；开发模式还会在 `127.0.0.1:24678` 占用 Vite 的 HMR 端口。
